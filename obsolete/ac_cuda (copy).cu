...
ACCUDA_STRUCT * acNew () {
  ...
  if (UsePinnedMem) {
    CUDA_SAFE_CALL_NO_SYNC(cudaMallocHost((void**)&ac, sizeof(ACCUDA_STRUCT)));
  } else {
    ac = (ACCUDA_STRUCT*)malloc(sizeof(ACCUDA_STRUCT));
  }

  memset(ac, 0, sizeof(ACCUDA_STRUCT));
  ac->acsm = acsmNew();
