...
int ValidateHeaderCheck(Rule *rule, HdrOptCheck *optData)
{
    ...
    case IP_HDR_OPTIONS:
        ...
        {
            ErrorMessage("Invalid operator for Check Header IP Options: %d "
    ...
    case IP_HDR_FRAGBITS:
        ...
        {
            ErrorMessage("Invalid operator for Check IP Fragbits: %d "
                "for dynamic rule [%d:%d].\n",
                optData->op, rule->info.genID, rule->info.sigID);
            retVal = -1;
        }
...
int checkOptions(uint32_t value, int op, Options options[], int numOptions)
{
    int found = 0;
    int i;
...
static int checkHdrOptInternal(void *p, HdrOptCheck *optData)
{
    ...
    if ((optData->hdrField & IP_HDR_OPTCHECK_MASK) && (!pkt->iph))
        return RULE_NOMATCH;
    ...
    case IP_HDR_FRAGOFFSET:
        ...
        break;
    case IP_HDR_TOS:
        //value = pkt->iph->ip_tos;
        value = GET_IPH_TOS(pkt);
        break;
    case IP_HDR_TTL:
        //value = pkt->iph->ip_ttl;
        value = GET_IPH_TTL(pkt);
        break;
    case IP_HDR_OPTIONS:
        return checkOptions(optData->value, optData->op, pkt->ip_options, pkt->ip_option_count);
        break;

    /* TCP Header checks */
    case TCP_HDR_ACK:
        value = ntohl(pkt->tcph->th_ack);
        break;
    case TCP_HDR_SEQ:
        value = ntohl(pkt->tcph->th_seq);
        break;
    case TCP_HDR_FLAGS:
        return checkBits(optData->value, optData->op, (pkt->tcph->th_flags & ~optData->mask_value));
        break;
    case TCP_HDR_WIN:
        value = ntohs(pkt->tcph->th_win);
        break;
    case TCP_HDR_OPTIONS:
        return checkOptions(optData->value, optData->op, pkt->tcp_options, pkt->tcp_option_count);
        break;

    /* ICMP Header checks */
    case ICMP_HDR_CODE:
        value = pkt->icmph->code;
        break;
    case ICMP_HDR_TYPE:
        value = pkt->icmph->type;
        break;
    case ICMP_HDR_ID:
        if ((pkt->icmph->code == ICMP_ECHO) ||
            (pkt->icmph->code == ICMP_ECHOREPLY))
        {
            value = ntohs(pkt->icmph->s_icmp_id);
        }
        else
        {
            return RULE_NOMATCH;
    ...
    case ICMP_HDR_SEQ:
        if ((pkt->icmph->code == ICMP_ECHO) ||
            (pkt->icmph->code == ICMP_ECHOREPLY))
        {
            value = ntohs(pkt->icmph->s_icmp_seq);
        }
        else
        {
            return RULE_NOMATCH;
