  ...
  switch( p->method )
   {
     ...
     case MPSE_ACB:
     case MPSE_ACSB:
        printf("Compile acsm w/o snort config\n");
                         ...
                         int ( *neg_list_func )(void *id, void **list) )
{
  int retv;
  MPSE * p = (MPSE*)pvoid;
  printf("====== PREPPATTERNS WITH SNORT CONFIG\n");
  ...
  switch( p->method )
   {
     ...
     case MPSE_ACB:
     case MPSE_ACSB:
        printf("Compile acsm WITH snort config\n");
...
#endif
        case XDFA_GPU:
            return xdfa_patterncount((struct xdfactx *)p->obj);
    }
    return 0;
}
