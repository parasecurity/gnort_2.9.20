...
void dfaerror PARAMS ((const char *msg)) {
	fprintf(stderr, "%s: %s\n", __func__, msg);
	return;
}
static ptr_t
...
/*
 * refresh the max number of states
 * (if it needs to be refreshed)
 */
int
get_max_state(struct dfa *d, int state, int max_state)
{
	int i;
	int max;
	...
	if (d->trans[state]) {
		for (i = 0; i < 256; i++)
			if (d->trans[state][i] > max)
				max = d->trans[state][i];
	}
	else if (d->fails[state]) {
		for (i = 0; i < 256; i++)
			if (d->fails[state][i] > max)
				max = d->fails[state][i];
	} else {
...
/*
 * prints transitions of a state
 * verbose
 *	0: do not print failing transitions
 *	1: print failing transitions
 */
void
print_state(struct dfa *d, int state, int verbose)
{
	int i;
	...
	if (d->trans[state]) {
		...
		for (i = 0; i < 256; i++) {
			...
			if ((i < 33) || (i > 126)) {
				if (verbose)
					printf("[.:%d]\t", d->trans[state][i]);
				else if ((d->trans[state][i] != -1) && (d->trans[state][i] != 0))
					printf("[.:%d]\t", d->trans[state][i]);
			} else {
				if (verbose)
					printf("[%c:%d]\t", i, d->trans[state][i]);
				else if ((d->trans[state][i] != -1) && (d->trans[state][i] != 0))
					printf("[%c:%d]\t", i, d->trans[state][i]);
			}
		}
		...
		for (i = 0; i < 256; i++) {
			...
			if ((i < 33) || (i > 126)) {
				if (verbose)
					printf("[.:%d]\t", d->fails[state][i]);
				else if ((d->fails[state][i] != -1) && (d->fails[state][i] != 0))
					printf("[.:%d]\t", d->fails[state][i]);
			} else {
				if (verbose)
					printf("[%c:%d]\t", i, d->fails[state][i]);
				else if ((d->fails[state][i] != -1) && (d->fails[state][i] != 0))
					printf("[%c:%d]\t", i, d->fails[state][i]);
			}
		}
...
/*
 * make the tans and fails arrays GPU compatible
 */
void
dfa_format_state_table(struct dfa *d)
{
	int i;
	int j;

	for (i = 0; i < d->trcount; i++) {
		if (d->trans[i]) {
			for (j = 0; j < 256; j++) {
				if (d->trans[i][j] == -1) {
					d->trans[i][j] = 0;
				}
				else if ((d->trans[i][j] > 0) && 
				    d->fails[d->trans[i][j]]  && 
				    d->success[d->trans[i][j]]) {
					d->trans[i][j] = -d->trans[i][j];
				}
			}
		}
		else if (d->fails[i]) {
			for (j = 0; j < 256; j++) {
				...
				else if ((d->fails[i][j] > 0) &&
				    d->fails[d->fails[i][j]]  &&
				    d->success[d->fails[i][j]]) {
					d->fails[i][j] = -d->fails[i][j];
				}
			}
		} else {
			printf("you shouldn't be here..\n");
...
dfabuild(struct dfa *d)
{
	...
	build_state_zero(d);
#ifdef PRINT_STATES
	print_state(d, state, 1);
#endif /* PRINT_STATES */

	/* if more states are found, keep building */
	max_state = get_max_state(d, state, max_state);
	...
	dfa_format_state_table(d);
}


#if 0
void dfabuild (struct dfa *d) {
	int i, j, state;
	int *trans;
	...
	for (i = 0; i < d->trcount; ++i) {
		...
		for (j = 0; j < 256; ++j) {
			...
			if (d->trans[state] != NULL) { ; }
			else if (state < 0) { ; }
			else if (d->fails[state]) { ; }
			else { build_state(state, d); }
		}
	}
	...
	for (i = 0; i < d->trcount; ++i) {
		for (j = 0; j < 256; ++j) {
			...
			else if (d->trans[i][j] > 0 && d->fails[d->trans[i][j]] && d->success[d->trans[i][j]]) { 
				d->trans[i][j] = -d->trans[i][j];
			}
		}
	}
...
void callback(void *user, const unsigned int offset);

int dfamatch_dispatch(struct dfa *d, unsigned char *buffer, unsigned int size, void (*callback)(void *user, const unsigned int offset), void *user) {
	unsigned int offset, index;
	int state_prev = 0, state = 0;
	unsigned char ch;

	for (index = 0; index < size; ++index) {
		ch = buffer[index];

		state = d->trans[state][ch];

		if (!state_prev && state)
			offset = index;  // potential match offset

		state_prev = state;

		if (state < 0) { // we have a match
			callback(user, offset);
			//fprintf(stderr, "match at %d ([%d][%d]) [prev. state: %d]\n", offset, state, ch, prev_state);
			//state = -state;
			state = 0;
		}
	}
...
int dfamatch(struct dfa *d, unsigned char *buffer, unsigned int size) {
	unsigned int offset, index;
	int state_prev = 0, state = 0;
	unsigned char ch;
...
int dfamatch_linear(int *trans, unsigned char *buffer, unsigned int size) {
        unsigned int offset, index;
        int state_prev = 0, state = 0;
        unsigned char ch;
	static int cnt = 0;
...
void dfaprint(struct dfa *d) {
	...
	for (i = 0; i < d->trcount; ++i) {
		...
		fprintf(stderr, "[%d]", i);
		for (j = 0; j < 255; j++) {
			if (d->trans[i][j] > 0 && d->fails[d->trans[i][j]] && d->success[d->trans[i][j]]) { fprintf(stderr, "!"); }
			fprintf(stderr, "%d ", d->trans[i][j]);
			if (d->trans[i][j] > 0 && d->fails[d->trans[i][j]] && d->success[d->trans[i][j]]) { fprintf(stderr, "!"); }
		}
		fprintf(stderr, "\n");
	}
...
void dfaaddpatt(struct dfa *d, char *pattern, unsigned int length) {

	if (d->regexp == NULL) {
		d->re_alloc = length + 4;
		d->re_count = length + 2;
		MALLOC(d->regexp, char, d->re_alloc);
		d->regexp[0] = '(';
		memcpy(&(d->regexp[1]), pattern, length * sizeof(char));
		d->regexp[length + 1] = ')';
		d->regexp[length + 2] = 0x00;
		return;
	}

	while (d->re_alloc - d->re_count < length + 4) {
		d->re_alloc = 2 * d->re_alloc;
		REALLOC(d->regexp, char, d->re_alloc);
	}

	d->regexp[d->re_count] = '|';
	d->regexp[d->re_count + 1] = '(';

	memcpy(&(d->regexp[d->re_count + 2]), pattern, length * sizeof(char));

	d->regexp[d->re_count + length + 2] = ')';

	d->regexp[d->re_count + length + 3] = 0x00;
