...
int fpSetDetectSearchMethod(FastPatternConfig *fp, char *method)
{
    ...
    else if( !strcasecmp(method,"xdfa-gpu") )
    {
        fp->search_method = XDFA_GPU;
        LogMessage("   Search-Method = xdfa-gpu\n");
    }
    else
    {
       return -1;
        ...
        OptTreeNode *otn, PatternMatchData *pmd_list, FastPatternConfig *fp)
{
    ...
    switch (pm_type)
    {
        case PM_TYPE__CONTENT:
            if (pmd_list == NULL){
                // printf("728: returned -1. fpcreate\n"); // breaks here
                return -1;
            }
        ...
        case PM_TYPE__HTTP_CLIENT_BODY_CONTENT:
            if (pmd_list == NULL){
                // printf("729: returned -1. fpcreate\n");
                return -1;
            }
        ...
        default:
            if (pmd_list != NULL){
                // printf("730: returned -1. fpcreate\n");
                return -1;
            }
    ...
    for (pmd = pmd_list; pmd != NULL; pmd = pmd->next)
    {
        ...
        printf("pattern in fpcreate: %s\n", pattern); // reaches here
...
static int fpFinishPortGroup(SnortConfig *sc, PORT_GROUP *pg, FastPatternConfig *fp)
{
    PmType i;
    int rules = 0;
    // printf("987: fpFinishPortGroup\n");

    if ((pg == NULL) || (fp == NULL)) {
        printf("472: pg or fp null.\n");
        return -1;
    }
    ...
    // mpsePrepPatternsWithSnortConf(sc, pg)
